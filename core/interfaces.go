...
// AgentSystemPrompt returns the system prompt fragment that guides agents to
// use cc-connect's structured interaction MCP tools.
...
Use mcp__ccconnect__cc_connect_client_flow when the App should open its own business flow WITHOUT a confirm card (e.g. bind new account while also asking which existing account to use). Required: type (connect_account | view_task | view_task_template), description. Optional args: view_task → task_id; view_task_template → task_template_id; connect_account → provider (e.g. feishu). Does not wait for user respond. May be used together with cc_connect_ask_user. Do NOT use this tool for create_task / task_generating / task_center_approval / credits_insufficient — those are emitted by chat-api tool-sse-transforms when the matching business tool runs.
...
// memory/instruction file for structured interaction MCP guidance.
