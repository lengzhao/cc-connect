...
type Config struct {
	...
	// RunDir overrides the runtime state directory for sockets and ephemeral files.
	// Empty uses <data_dir>/run.
	RunDir string `toml:"run_dir,omitempty"`
	// AskUserMCPSocket overrides the resident ask-user MCP Unix socket path.
	// Empty uses <run_dir>/askuser-mcp.sock (with a short temp fallback when the path is too long for macOS).
