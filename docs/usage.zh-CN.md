...
| `/mode [名称]` | 查看或切换权限模式 |
| `/stop` | 停止当前执行 |
| `/send <群ID> <消息>` | 向其他群/会话发送消息（需 `admin_from`；支持多行） |
| `/help` | 显示可用命令 |
...
无需通过 shell 调用 `cc-connect send`，即可向同平台的其他群/会话发送消息：
...
- `<群ID>` 为平台原生群/会话 ID（飞书 `oc_xxx` 等）。
- 需要配置 `admin_from`（与 `/shell`、`/dir` 相同）。
- 同一条消息内的多行文本会作为一条 outbound 消息发送。
- 内置 `/send` 优先于名为 `send` 的自定义 `[[commands]]`；建议删除旧配置以免混淆。
