...
| `event` | string | 否 | 导航引导字符串，原样透传给 App（常见值：`connect_account` / `create_task` / `task_generating` / `task_center_approval` / `credits_insufficient`）。缺省或空时不出现该字段，前端不渲染额外功能按钮，走通用发送/确认 |
...
# response_header_value = "pod-a"                    # 固定值
# response_header_env = "POD_NAME"                   # 或启动时从环境变量读取（value 优先）
