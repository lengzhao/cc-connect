...
`client_flow` carries `flow_id`, `type`, `description`, `run_id`, and `message_id`, plus optional `args`. Sources: (1) MCP `cc_connect_client_flow` — Agent-invoked; schema enum is only `connect_account` / `view_task` / `view_task_template`; (2) `tool_sse_transforms_file` — programmatic for business tools (`create_task` / `task_generating` / `task_center_approval` / `credits_insufficient`, etc.); (3) chat-api resume may emit `waiting_answer` when a question is still pending. Semantics of `args` depend on `type`: task-related types use `task_id`; `view_task_template` uses `task_template_id`; `connect_account` uses `provider` (e.g. `feishu`). It is non-blocking: it creates no interaction, occupies no interaction slot, requires no respond, and may coexist with `question_request`. The App handles the type locally while the SSE turn keeps streaming. See [Client flow MCP](./plans/2026-07-23-chat-api-client-flow-design.md).
