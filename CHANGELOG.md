...
- **chat-api question notify**: detached `question_request` webhook body is simplified to `conversation_id`, `message_id`, `run_id`, `user_id`, and `channel`; optional `question_notify_headers` supplies custom outbound headers for BFF/gateway integration.
