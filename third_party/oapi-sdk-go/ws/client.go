...
// Close stops the WebSocket client gracefully.
// It disables auto-reconnect and closes the active connection so that
// Feishu's push service removes this endpoint from its routing table promptly.
func (c *Client) Close() {
	c.mu.Lock()
	...
	for {
		if c.conn != nil {
			...
			if err != nil {
				...
				c.disconnect(ctx) // broken connection, trigger reconnect
			} else {
...
func (c *Client) receiveMessageLoop(ctx context.Context) {
	...
	for {
		...
		// Set read deadline to detect half-dead connections.
		// A healthy connection resets this via pong; a dead one times out here.
		if err := c.conn.SetReadDeadline(time.Now().Add(3 * c.pingInterval)); err != nil {
			c.logger.Warn(ctx, c.fmtLog("set read deadline failed, err: %v", err)...)
		}
...
func (c *Client) handleControlFrame(ctx context.Context, frame Frame) {
	...
	case MessageTypePong:
		...
		// Renew read deadline — connection is alive.
		c.mu.Lock()
		conn := c.conn
		c.mu.Unlock()
		if conn != nil {
			_ = conn.SetReadDeadline(time.Now().Add(3 * c.pingInterval))
		}
