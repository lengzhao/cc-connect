...
// TestContextResourcesIncludesAutomonSkillsMemoryAndKnowledgeNoAgentWorkDir tests
// the fallback behaviour when AGENT_WORK_DIR is not set: files are loaded from
// the agent's own workDir.
func TestContextResourcesIncludesAutomonSkillsMemoryAndKnowledgeNoAgentWorkDir(t *testing.T) {
	workDir := t.TempDir()
	configDir := filepath.Join(t.TempDir(), ".claude")
	t.Setenv("CLAUDE_CONFIG_DIR", configDir)
	automonPath := filepath.Join(configDir, "AUTOMON.md")
